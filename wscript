...
def options(opt):
	...
	grp.add_option('--enable-speex', action = 'store_true', dest = 'SPEEX', default = False,
		help = 'build engine with Speex voice codec [default: %default]')
...
def configure(conf):
	...
	if conf.options.SPEEX or conf.env.DEST_OS == 'android':
		projects['game'] += ['engine/voice_codecs/speex']
...
def build(bld):
	...
	if bld.env.SPEEX or bld.env.DEST_OS == 'android':
		projects['game'] += ['engine/voice_codecs/speex']
