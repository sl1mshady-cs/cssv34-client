...
void CNetChan::CompressFragments()
{
	...
	for ( int i=0; i<MAX_STREAMS; i++ )
	{
		...
		if ( data->buffer )	
		{
			...
			unsigned int compressedSize = NET_MAX_PAYLOAD_V23;
			char * compressedData = new char[ compressedSize ];

			if ( BZ2_bzBuffToBuffCompress( compressedData, &compressedSize, data->buffer, data->bytes, 900, 0, 0 ) &&
				( compressedSize < data->bytes ) )
			{
				compressTimer.End(); 
				DevMsg("Compressing fragments (%d -> %d bytes): %.3fms\n",
...
void CNetChan::UncompressFragments( dataFragments_t *data )
{
	...
	// uncompress data
	BZ2_bzBuffToBuffDecompress( newbuffer, &uncompressedSize, data->buffer, data->bytes, 0, 0 );
...
bool CNetChan::SendSubChannelData( bf_write &buf )
{
	...
	for ( i=0; i<MAX_STREAMS; i++ )
	{
		...
		{	
			...
			buf.WriteUBitLong( data->bytes, NET_MAX_PAYLOAD_BITS_V23 );
		}
		else
		{
...
bool CNetChan::ReadSubChannelData( bf_read &buf, int stream  )
{
	...
	if ( offset == 0 ) // first fragment, read header info
	{
		...
		if ( bSingleBlock )
		{
			...
			data->bytes = buf.ReadUBitLong(NET_MAX_PAYLOAD_BITS_V23);
		}
		else
		{
...
int CNetChan::SendDatagram(bf_write *datagram)
{
	...
	// Send the datagram
	int	bytesSent = NET_SendPacket ( this, m_Socket, remote_address, send.GetData(), send.GetNumBytesWritten(), bSendVoice ? &m_StreamVoice : 0, false );
	...
	if ( net_showudp.GetInt() && net_showudp.GetInt() != 2 )
	{
		...
		ConMsg ("UDP -> %12.12s: sz=%5i seq=%5i ack=%5i rel=%1i tm=%f rt=%f\n"
			, GetName()
