...
int get_host_identifier()
{
	...
	{
		// fallback
		id = (int)GetCurrentProcessId() ^ (int)GetTickCount();
	}
	...
	if (gethostname(hostname, sizeof(hostname)) == 0)
	{
		...
		{
			hash = (hash << 5) + hash + hostname[i]; // hash * 33 + c
		}
	...
	{
		// Fallback если gethostname не сработал
		id = (int)getpid() ^ (int)time(NULL);
	}
#endif
...
int GenerateTicket(char* buf, int maxsize)
{
	int volumeSerial = get_host_identifier();
	...
	Msg("Your SteamID: STEAM_0:0:%i\n", hash);

	char szhwid[64];

	CreateRandomString(szhwid, 16);

	if (!RevSpoofer::Spoof(szhwid, hash))
		return 0;

	auto pTicket = (int*)buf;
...
bool CBaseClientState::PrepareSteamConnectResponse( uint64 unGSSteamID, bool bGSSecure, const netadr_t &adr, bf_write &msg )
{
	...
	uint32 steam3CookieLen = GenerateTicket(steam3Cookie, STEAM_KEYSIZE);
...
void CBaseClientState::CheckForResend (void)
{
	...
	// Request another challenge value.
	{
		...
		msg.WriteString( "00000000000000" ); // padd out
		NET_SendPacket( NULL, m_Socket, adr, msg.GetData(), msg.GetNumBytesWritten() );
	}
}
