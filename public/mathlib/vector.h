...
public:
	...
	inline Quaternion( DegreeEuler const &angle );
...
public:
	...
	// RuSHeRR NOTE: csgo had explicit in here, removed due to compatibility issues.
	inline RadianEuler( Quaternion const &q );
	inline RadianEuler( QAngle const &angles ); 
	inline explicit RadianEuler( DegreeEuler const &angles ); // explicit will stay here only!
...
// Zero the object -- necessary for CNetworkVar and possibly other cases.
FORCEINLINE void NetworkVarConstruct(Quaternion& q) { q.Init(); }
...
// Zero the object -- necessary for CNetworkVar and possibly other cases.
FORCEINLINE void NetworkVarConstruct( QAngle &x ) { x.Init(); }
...
inline const Vector QuaternionLog( const Quaternion &q )
{
	...
	if ( sinTheta > 0.001f )
	{
		...
		factor = ( q.w < 0.0f ? 3.14159265358979323846 - theta : theta ) / sinTheta;
	}
	else
	{
