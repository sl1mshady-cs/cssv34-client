...
void CBaseGamesPage::UpdateStatus()
{
	if (m_pGameList->GetItemCount() > 1)
	{
		...
		_snwprintf( msg, Q_ARRAYSIZE(msg), L"%ls( %d )", g_pVGuiLocalize->Find( "#ServerBrowser_Servers"), m_pGameList->GetItemCount() );
...
void CBaseGamesPage::OnViewGameInfo()
{
	...
	// join the game
	ServerBrowserDialog().OpenGameInfoDialog(this, GetServer(serverID));
}
